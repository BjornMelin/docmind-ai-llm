...
| :--- | :--- | :--- |
| **Baseline** (`uv sync`) | Vector + hybrid retrieval | GraphRAG paths may be requested/configured, but the router will remain vector-only when the GraphRAG adapter cannot be registered (missing graph-store deps, health check failures). A single guidance warning is emitted when GraphRAG is requested but unavailable. |
| **GraphRAG extra** (`uv sync --frozen --extra graph`) | Property-graph retrieval (Kùzu-backed graph store) | The default GraphRAG adapter can register, enabling the `knowledge_graph` tool when a property graph index is present and healthy. |
| **Multimodal extra** (`uv sync --frozen --extra multimodal`) | ColPali reranker + vision deps | Optional lane for image-heavy workloads; behavior downgrades when absent. |
