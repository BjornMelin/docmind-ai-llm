...
def test_setup_llamaindex_success() -> None:
    ...
    with (
        ...
        patch("src.config.integrations.ClipEmbedding") as mock_embed,
    ):
        mock_llm = object()
...
def test_setup_llamaindex_failure_paths() -> None:
    ...
    with (
        ...
        patch("src.config.integrations.ClipEmbedding", side_effect=RuntimeError("x")),
    ):
        from llama_index.core import Settings
        ...
        embed_ok = (Settings.embed_model is None) or (
            ...
            in {"MockEmbedding", "ClipEmbedding"}
        )
        assert llm_ok
        assert embed_ok
