...
def test_settings_default_values():
    """Test AppSettings model loads with expected default values.

    Tests that AppSettings model initializes with correct default
...
def test_settings_environment_override():
    """Test AppSettings model respects environment variable overrides.

    Tests that AppSettings model properly uses environment variables
...
def test_app_settings_document_processing_defaults():
    """Test AppSettings document processing configuration defaults.

    Tests that AppSettings model initializes with correct default
    values for document processing and parsing configuration.
    """
    settings = AppSettings()
    assert settings.parse_strategy == "hi_res"
...
def test_gpu_acceleration_settings():
    ...
    Verifies GPU settings for embedding batch processing,
    CUDA device selection, and performance optimization.
    """
    settings = AppSettings()
    ...
    with pytest.raises(ValidationError):
        AppSettings(embedding_batch_size=0)  # Should be >= 1

    with pytest.raises(ValidationError):
        AppSettings(embedding_batch_size=1000)  # Should be <= 512


def test_quantization_settings():
    """Test scalar quantization configuration.

    Verifies quantization settings for memory optimization
    with proper type validation.
    """
    settings = AppSettings()

    assert settings.enable_quantization is True
    assert settings.quantization_type == "int8"
    assert settings.max_concurrent_requests == 10

    # Test concurrent requests validation
    with pytest.raises(ValidationError):
        AppSettings(max_concurrent_requests=0)

    with pytest.raises(ValidationError):
        AppSettings(max_concurrent_requests=200)  # Should be <= 100


def test_reranking_configuration():
    """Test ColBERT reranking configuration.

    Verifies reranking model selection and top-k settings
    for optimal retrieval performance.
    """
    settings = AppSettings()

    assert (
        settings.default_reranker_model == "jinaai/jina-reranker-v2-base-multilingual"
    )
    ...
    with pytest.raises(ValidationError):
        AppSettings(reranking_top_k=3)  # Should be >= 5

    with pytest.raises(ValidationError):
        AppSettings(reranking_top_k=200)  # Should be <= 100


def test_document_processing_configuration():
    """Test enhanced document processing settings.

    Verifies Unstructured parsing strategy, chunk size optimization,
    and knowledge graph entity extraction limits.
    """
    settings = AppSettings()
    ...
    with pytest.raises(ValidationError):
        AppSettings(chunk_size=100)  # Should be >= 256

    with pytest.raises(ValidationError):
        AppSettings(chunk_size=8192)  # Should be <= 4096

    # Test entity count validation
    with pytest.raises(ValidationError):
        AppSettings(max_entities=0)

    with pytest.raises(ValidationError):
        AppSettings(max_entities=300)  # Should be <= 200


def test_default_model_configuration():
    """Test hardware-adaptive default model configuration.

    Verifies the updated default model setting for hardware adaptation.
    """
    settings = AppSettings()
...
def test_qdrant_url_configuration():
    ...
    Verifies the updated Qdrant URL default for hardware adaptation.
    """
    settings = AppSettings()

    assert (
        settings.qdrant_url == "http://localhost:7000"
    )  # Updated default for hardware adaptation
...
def test_legacy_compatibility():
    """Test legacy settings compatibility.

    Verifies that legacy embedding model settings are maintained
    for backward compatibility while new settings take precedence.
    """
    settings = AppSettings()

    # Test legacy default embedding model
    assert settings.default_embedding_model == "jinaai/jina-embeddings-v4"

    # Verify new dense embedding model takes precedence
    assert settings.dense_embedding_model == "BAAI/bge-large-en-v1.5"

    # Test alias functionality
    with patch.dict(os.environ, {"DEFAULT_EMBEDDING_MODEL": "custom/model"}):
        settings = AppSettings()
...
def test_model_post_init_compatibility_alias():
    """Test model_post_init sets up compatibility aliases correctly."""
    # Test default behavior
    settings = AppSettings()
    settings.model_post_init({})  # Manually call to test
    assert settings.reranker_model == settings.default_reranker_model

    # Test with custom reranker model (should preserve custom value)
    custom_settings = AppSettings(reranker_model="custom/reranker-v2")
    assert custom_settings.reranker_model == "custom/reranker-v2"
    assert custom_settings.reranker_model != custom_settings.default_reranker_model
