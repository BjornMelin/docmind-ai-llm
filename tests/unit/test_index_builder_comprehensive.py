...
class TestCreateHybridRetriever:
    ...
    def test_create_hybrid_retriever_exception_fallback(self, sample_documents):
        ...
        with (
            ...
            # Should be called twice - once for each retriever in try block,
            # once for fallback
    ...
    def test_create_hybrid_retriever_settings_configuration(self):
        ...
        with (
            ...
            fusion_call = mock_fusion.call_args[1]
            assert fusion_call["similarity_top_k"] == test_settings.similarity_top_k
...
class TestCreateIndexAsync:
    ...
    async def test_create_index_async_success(self, sample_documents):
        ...
        with (
            patch("utils.index_builder.managed_async_qdrant_client") as mock_client_ctx,
            ...
            patch("utils.index_builder.VectorStoreIndex") as mock_index_cls,
            patch("utils.index_builder.create_hybrid_retriever") as mock_retriever,
        ):
            # Arrange
            ...
            with (
                patch("utils.index_builder.KnowledgeGraphIndex") as mock_kg_cls,
                patch("utils.index_builder.ensure_spacy_model"),
                patch("utils.index_builder.Ollama"),
            ):
                mock_kg_cls.from_documents.return_value = mock_kg_index
    ...
    async def test_create_index_async_gpu_enabled(self, sample_documents):
        ...
        with (
            ...
            patch("utils.index_builder.managed_async_qdrant_client") as mock_client_ctx,
            patch("utils.index_builder.managed_gpu_operation") as mock_gpu_ctx,
        ):
            # Arrange mocks
            ...
            with (
                ...
                patch("utils.index_builder.settings") as mock_settings,
            ):
    ...
    async def test_create_index_async_kg_failure_fallback(self, sample_documents):
        ...
        with (
            ...
            with (
                ...
                patch("utils.index_builder.ensure_spacy_model"),
            ) as (_, mock_ensure_spacy):
                # Act
                result = await create_index_async(sample_documents, use_gpu=False)

                # Assert - should still return vector index
                assert result["vector"] == mock_index
                assert result["kg"] is None  # Should be None due to failure
    ...
    async def test_create_index_async_retriever_failure_fallback(
        ...
        with (
            ...
            with (
                ...
                # Assert - should still return vector index
                assert result["vector"] == mock_index
                assert result["retriever"] is None  # Should be None due to failure
...
class TestCreateMultimodalIndex:
    ...
    def test_create_multimodal_index_success(self):
        ...
        with (
            ...
            patch("utils.index_builder.MultiModalVectorStoreIndex") as mock_mm_index,
        ):
            mock_index = MagicMock()
    ...
    def test_create_multimodal_index_no_text_docs_raises(self):
        ...
        with (
            ...
            pytest.raises(Exception, match="No text documents available for fallback"),
        ):
    ...
    def test_create_multimodal_index_quantization_enabled(self):
        ...
        with (
            patch("utils.index_builder.settings") as mock_settings,
            patch("transformers.BitsAndBytesConfig") as mock_quant_config,
            patch(
...
class TestCreateMultimodalIndexAsync:
    ...
    async def test_create_multimodal_index_async_success(self):
        ...
        with (
            ...
            # Act
            result = await create_multimodal_index_async(mixed_docs, use_gpu=True)
    ...
    async def test_create_multimodal_index_async_gpu_operations(self):
        ...
        with (
            ...
            # Act
            await create_multimodal_index_async(docs, use_gpu=True)
