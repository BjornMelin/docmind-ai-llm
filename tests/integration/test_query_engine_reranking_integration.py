...
class TestQueryEngineRerankingIntegration:
    ...
    def test_hybrid_search_with_reranking_pipeline(
        ...
        # Create reranker
        reranker = BGECrossEncoderRerank(top_n=5, normalize_scores=False)
    ...
    async def test_async_query_engine_reranking_workflow(
        ...
        reranker = BGECrossEncoderRerank(top_n=2, normalize_scores=False)
    ...
    def test_reranking_performance_with_query_routing(
        ...
        reranker = BGECrossEncoderRerank(top_n=5, batch_size=16, normalize_scores=False)
...
class TestQueryEngineRerankingMemoryEfficiency:
    ...
    def test_large_document_set_reranking(
        ...
        for i in range(100):
            large_nodes.append(
                NodeWithScore(
                    node=TextNode(
                        text=(
                            f"Large document {i} with extensive content about machine "
                            "learning"
                        ),
                        id_=f"large_doc_{i}",
                    ),
        ...
        result = reranker.postprocess_nodes(large_nodes, query_bundle)
