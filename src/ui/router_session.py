...
def replace_session_router(
    ...
    *,
    ...
    if previous is router and session_state.get(_ROUTER_GENERATION_KEY) == generation:
        return
    ...
    with _REGISTRY_LOCK:
        ...
        if router is not None:
            try:
                _REGISTERED_ROUTERS[id(router)] = router
            except TypeError:
                _NON_WEAK_ROUTERS[id(router)] = router
        session_state["router_engine"] = router
        session_state[_ROUTER_GENERATION_KEY] = generation

    if previous is not _MISSING and previous is not router:
        _close_router(previous)
